...
class ColorFormatter(logging.Formatter):
    ...
    MESSAGE_COLOR_BY_FILE = (
        {}
    )  # Custom color per file, which gets added to by helper function

    def format(self, record):
        log_color = self.COLORS.get(
            record.levelno, Style.RESET_ALL
...
def set_files_message_color(color_name):
    ...
    color = COLOR_MAP.get(
        color_name.upper(), Style.RESET_ALL
    )  # Get color from COLOR_MAP if it exists otherwise default white
    ...
    if color_formatter:
        ...
        logger.info(
            f"Set message color for {caller_filename} to {color_name.upper()}"
        )  # Log the file color set to change
    else:
