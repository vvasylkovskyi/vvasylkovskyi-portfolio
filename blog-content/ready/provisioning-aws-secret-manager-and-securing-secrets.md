...
The `assume_role_policy` (also called trust policy) defines _WHO_ can use this role.
...
Now, we create a policy that defined _WHAT_ the role can do:
...
Now, make the ec2 instance fetch the secrets at runtime using the IAM role.
...
```hcl
  user_data = <<-EOF
              ...
              export API_KEY=$(echo "$SECRET" | jq -r '.my_api_key')
